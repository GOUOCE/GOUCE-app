...
class CriarUsuarioUseCase:
    ...
    def execute(self, dto: CadastroUsuarioDTO) -> CadastroSucessoDTO:
        ...
        if not email:
            erros.append(CadastroValidationError("email", "E-mail é obrigatório"))
        elif not self.email_validator.validar_email(email):
            erros.append(CadastroValidationError("email", "Formato de e-mail inválido"))
        ...
        if dto.data_nascimento is not None and dto.data_nascimento != "" and dto.data_nascimento != 0:
            try:
                ...
                erros.append(CadastroValidationError("data_nascimento", str(e)))
        else:
        ...
        if dto.periodo_ingresso and str(dto.periodo_ingresso).strip():
            try:
                ...
                erros.append(CadastroValidationError("periodo_ingresso", str(e)))
        ...
        if not dto.faculdade_id or len(str(dto.faculdade_id).strip()) < 2:
            erros.append(CadastroValidationError("faculdade_id", "Instituição/Faculdade é obrigatória"))
        ...
        if not dto.id_comprovante_residencia or not str(dto.id_comprovante_residencia).strip():
            erros.append(CadastroValidationError("id_comprovante_residencia", "Comprovante de residência é obrigatório"))
        ...
        if self.arquivo_repository:
            ...
            if dto.id_foto_aluno and not self.arquivo_repository.buscar_por_id(str(dto.id_foto_aluno)):
                erros.append(CadastroValidationError(
                    "id_foto_aluno",
                    "A foto de perfil informada não é um arquivo válido registrado no sistema",
                ))

        # 13. Validação de Termos de Uso
        if not dto.termos_de_uso:
            erros.append(CadastroValidationError("termos_de_uso", "Você deve aceitar os termos de uso para se cadastrar"))
